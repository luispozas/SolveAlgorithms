...
public:
   ...
   std::pair<bool,T> lower_bound(T const& e){
      return lower_bound_private(e, raiz);
   }
...
protected:
   ...
   std::pair<bool,T> lower_bound_private(T const& e, Link a) const {
      if (a == nullptr) {
         return {false, e};
      }
      else{
         std::pair<bool, T> p;
         if (menor(e, a->elem)) {
            p = lower_bound_private(e, a->iz);
            if(!p.first) return {true, a->elem};
            else return p;
         }
         else if (menor(a->elem, e)) {
            return lower_bound_private(e, a->dr);
         }
         else return {true, e};
      }
   }
